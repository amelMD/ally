...
let handler = async (m, { conn, args, usedPrefix }) => {
  ...
  if (!args[0]) {
    return conn.reply(m.chat, `❓ Kamu belum memasukkan Kode redeemcode!\n\nContoh: *${usedPrefix}redeemcode code*`, m);
  }
  ...
  if (validGiftCodes.includes(args[0])) {
    ...
    conn.reply(m.chat, `*❌ KODE FREE TIDAK VALID  ❌*\nSilakan periksa kembali Kode redeemcode yang kamu masukkan.\n\nContoh: *${usedPrefix}freegift code*`, m);
  }
};

handler.help = ['redeemcode <kode>'];
handler.tags = ['rpg'];
