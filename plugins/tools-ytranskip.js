...
const cheerio = require('cheerio'); // Import cheerio dengan benar
...
async function YoutubeTranscript(youtubeUrl) {
  try {
    ...
    const targetUrl = `https://youtubetotranscript.com/transcript?v=${videoId}¤t_language_code=en`; // Perbaiki template literal
    const { data } = await axios.get(targetUrl, {
      headers: { 'user-agent': 'Mozilla/5.0' },
    });
    const $ = cheerio.load(data);
    const title = $('h1.card-title').text().trim() || '';
    const author = $("a[data-ph-capture-attribute-element='author-link']")
      .text()
      .trim() || '';
    const thumbnail = `https://i.ytimg.com/vi/${videoId}/maxresdefault.jpg`; // Perbaiki template literal
    const transcriptArr = [];
