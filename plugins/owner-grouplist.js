...
const handler = async (m, { conn, usedPrefix, args }) => {
    ...
    if (args.length === 0) {
        // Menampilkan daftar grup dengan format yang disesuaikan
        const list = groups.map((group, index) => {
            const time = formatTime(group.creation);
            const status = group.restrict ? 'FOREVER' : 'NOT SET'; // Sebagai contoh, bisa disesuaikan
            const totalMembers = group.participants.length;
            const onTime = formatTime(new Date()); // Waktu terakhir (sementara diatur sekarang)
            return `› ${index + 1}. ${group.subject}\n🏴 : ${group.id}\n${status} | ${totalMembers} | ON | ${onTime}`;
        }).join('\n\n');
        
        conn.reply(m.chat, `╭──❲ ＧＲＯＵＰ - ＬＩＳＴ ❳\n│\n│ "Bot has joined into ${groups.length} groups, send gc or gcopt to show all setup options."\n│\n${list}\n╰───`, m);
