...
const handler = async (m, {
    ...
    if (args.length < 1) return m.reply("Format pesan salah. Contoh penggunaan: rodotz-pushgc <delay>|<idgroup>|<pesan>");
    
    let [delayStr, groupId, text] = args.join(' ').split('|');
    
    if (!groupId || !text) return m.reply("Format pesan salah. Contoh penggunaan: rodotz-pushgc <delay>|<idgroup>|<pesan>");
...
handler.help = ['rodotz-pushgc'];
handler.tags = ['pushkontak'];
handler.command = ['rodotz-pushgc'];
handler.owner = true;
