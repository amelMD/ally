...
const { generateWAMessageFromContent, generateWAMessage } = require('@whiskeysockets/baileys'); // Pastikan Anda menginstal library ini
...
async function sendAlbumMessage(jid, array, quoted, sock) {
...
const handler = async (m, { conn, text }) => {
    ...
    try {
        ...
        // Ganti bagian ini dengan pemanggilan fungsi sendAlbumMessage
        if (images.length > 0) {
            // Format data untuk album
            const albumImages = images.map(img => ({ image: { url: img } }));

            try {
                // Panggil fungsi sendAlbumMessage yang baru ditambahkan
                await sendAlbumMessage(m.chat, albumImages, m, conn);
            } catch (error) {
                console.error("Error sending album message:", error);
                // Sebagai fallback, kirim gambar satu per satu jika sendAlbumMessage gagal
                for (const img of images) {
                    await conn.sendMessage(m.chat, { image: { url: img } }, { quoted: m });
                }
            }
             try {
                 await conn.sendMessage(m.chat, { audio: { url: ressol.downloadUrls.music }, mimetype: 'audio/mpeg' }, { quoted: m });
             } catch (error) {
