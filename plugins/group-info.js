...
let handler = async (m, { conn, participants, groupMetadata }) => {
    const pp = await conn.profilePictureUrl(m.chat, 'image').catch(_ => null) || './src/pp.png'
    const { isBanned, welcome, detect, sWelcome, sBye, sPromote, sDemote, antiLink, antiDelete, antiBot, antiVideo, antiFoto, antiSticker, antiToxic, simi, nsfw, rpg, viewonce  } = global.db.data.chats[m.chat]
...
Anti Viewonce: ${viewonce ? 'Aktif' : 'Nonaktif'}
`.trim()
    conn.sendFile(m.chat, pp, 'pp.jpg', text, m, false, { mentions: [...groupAdmins.map(v => v.id), owner] })
}

handler.help = ['infogrup','infogc','gcinfo']
handler.tags = ['group']
