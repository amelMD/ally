...
let handler = async (m, { conn, args, usedPrefix, text, command }) => {
  ...
  if (command === 'buysewa' && link && !durasi) { // Ubah kondisi ini
    ...
    const sections = [
      {
        title: 'Durasi Sewa',
        rows: [
          { title: `2 Hari - Rp${harga.daily.toLocaleString()}`, rowId: `${usedPrefix}buysewa ${link},2d` },
          { title: `7 Hari - Rp${harga.weekly.toLocaleString()}`, rowId: `${usedPrefix}buysewa ${link},7d` },
          { title: `30 Hari - Rp${harga.monthly.toLocaleString()}`, rowId: `${usedPrefix}buysewa ${link},30d` },
          { title: `Custom (Rp${harga.custom.toLocaleString()}/hari)`, rowId: `${usedPrefix}buysewa ${link},10d` }
        ]
      }
    ];
    ...
    const listMessage = {
      ...
      footer: 'Pilih salah satu durasi di bawah ini.',
      title: "Pilihan Durasi Sewa",
      buttonText: "Pilih Durasi",
      sections
    }
  ...
  if (command === 'buysewa' && !link) return m.reply(`⚠️ Format salah!\n\n*Contoh:* ${usedPrefix}buysewa <link_grup>,1d`);
  if (!link.includes("whatsapp.com")) return m.reply("❌ Link GC tidak valid");

  let code = link.replace("https://chat.whatsapp.com/", "").trim(); // Ekstrak kode grup
  ...
  // === pesan loading ===
  let loadingMsg = await m.reply("⏳ Membuat invoice...");

  // === bikin invoice ===
  // Perbaikan: URL yang benar harus dimasukkan di sini
  ...
  if (!trx.qr_string) return m.reply("⚠️ QR String tidak tersedia dari server!");

  let qrBuffer;
  try {
    qrBuffer = await QRCode.toBuffer(trx.qr_string, { type: "png" });
  } catch (e) {
    console.error("Gagal membuat QR dari string:", e); // Log error
