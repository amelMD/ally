...
prefix = new RegExp('^[' + (opts['prefix'] || 'â€ŽxzXZ/!#$%+Â£Â¢â‚¬Â¥^Â°=Â¶âˆ†Ã—Ã·Ï€âˆšâœ“Â©Â®:;?&.\\-').replace(/[|\\{}()[\]^$+*?.\-\^]/g, '\\$&') + ']')
...
const authFile = `${opts._[0] || 'Ally-Sessions'}`
...
/** Apa Gunanya ini di tambah mas Rodokk? 🤨 **/
const getMessage = async key => {
	const messageData = await store.loadMessage(key.remoteJid, key.id);
	return messageData?.message || messageData.message
}
...
async function connectionUpdate(update) {
	...
	"⚡ Mengaktifkan Bot, Mohon tunggu sebentar..."
	));
	if (connection == "open") console.log(chalk.green(
	"✅ Bot telah terhubung"
	))
	if (connection == "close") console.log(chalk.red(	
	"⏱️Koneksi terputus dan mencoba menyambung ulang..."
	));
	...
	// console.log(JSON.stringify(update, null, 4))
	 ...
	 if((usePairingCode || useMobile) && fs.existsSync('./Ally-Sessions/creds.json') && !conn.authState.creds.registered) {
	 ...
	 if(usePairingCode && !conn.authState.creds.registered) {
	    ...
	    const phoneNumber = await question(chalk.bgBlack(chalk.whiteBright("Start with your country's WhatsApp code, Example : 62xxx\n")))
