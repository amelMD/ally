...
exports.makeWASocket = (connectionOptions, options = {}) => {
  ...
  /**
    ...
    */
    ...
    conn.sendContact1 = async (jid, contact, quoted, info = {}, opts = {}) => {
    let list = []
    contact.map(v => list.push({
      displayName: v.name,
      vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:${v.name}\nORG:${info && info.org ? info.org : 'Customer Support'}\nTEL;type=CELL;type=VOICE;waid=${v.number}:${PhoneNumber('+' + v.number).getNumber('international')}\nEMAIL;type=Email:${info && info.email ? info.email : 'contact me'}\nURL;type=Website:${info && info.website ? info.website : '-'}\nADR;type=Location:;;Unknown;;\nOther:${v.about}\nEND:VCARD`
    }))
...
`.trim()
     ...
     * @returns
     */
    conn.appenTextMessage = async (m, text, chatUpdate) => {
 ...
 exports.smsg = (conn, m, hasParent) => {
    ...
    if (m.message) {
       ...
       m.quoted.isBaileys = m.quoted.id ? m.quoted.sentFrom === 'web' || m.quoted.sentFrom === 'unknown' : false;
           // m.quoted.isBaileys = m.quoted.id ? (m.quoted.id.startsWith('B1EY') && m.quoted.id.length === 20) : false
