...
const fs = require('fs');
const __filename = new URL('', const.meta.url).pathname
const __dirname = new URL('.', const.meta.url).pathname
let file = fileURLToPath(const.meta.url)
fs.watchFile(file, () => {
    fs.unwatchFile(file)
    console.log(chalk.bgGreen(chalk.black("[  UPDATE ]")), chalk.white(`${__filename}`))
    const(`${file}?update=${Date.now()}`)
})